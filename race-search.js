...
exports.handler = async (event) => {
  ...
  console.log(`[timing-race] verifyAuth levou ${Date.now() - _t0}ms`);
...
- Só retorne {"races":[]} se realmente não existir nenhuma prova futura dessa modalidade.`;
  ...
  return new Promise((resolve) => {
    ...
    const req = https.request({
      ...
      res.on('end', () => {
        console.log(`[timing-race] chamada a Anthropic levou ${Date.now() - _tAnthropicStart}ms (total ate aqui: ${Date.now() - _t0}ms), HTTP ${res.statusCode}`);
        let parsed;
        try { parsed = JSON.parse(data); }
        catch (e) {
          console.log(`[timing-race] resposta nao-JSON, corpo bruto (300 chars): ${data.slice(0,300)}`);
        ...
        if (parsed.type === 'error' || parsed.error) {
          ...
          console.log(`[timing-race] erro da Anthropic:`, res.statusCode, msg);
          return resolve({ statusCode: 200, headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ races: [], error: 'API Anthropic (HTTP ' + res.statusCode + '): ' + msg }) });
        }
        if (res.statusCode !== 200) {
          console.log(`[timing-race] HTTP inesperado da Anthropic: ${res.statusCode}`);
          return resolve({ statusCode: 200, headers: { 'Content-Type': 'application/json' },
        ...
        // Conta quantas buscas o modelo realmente executou (server_tool_use com name web_search)
        // vs as ate 6 permitidas — diz se ele esta parando cedo demais.
        const searchesUsed = (parsed.content || []).filter(b => b.type === 'server_tool_use' && b.name === 'web_search').length;
        console.log(`[timing-race] stop_reason=${stopReason} buscas_executadas=${searchesUsed}/6 tamanho_texto=${text.length}`);

        const obj = extractRacesJson(text);
        let races = (obj && Array.isArray(obj.races)) ? obj.races : [];
        console.log(`[timing-race] JSON extraido: ${obj ? 'sim' : 'nao'}, provas cruas antes do filtro: ${races.length}`);
        ...
        console.log(`[timing-race] provas apos filtro de data/formato: ${races.length}`);
        ...
        knownRacesPromise.then(known => {
          console.log(`[timing-race] provas conhecidas (cache) encontradas: ${known.length}`);
          ...
          console.log(`[timing-race] total final mesclado: ${merged.length}, total da function: ${Date.now() - _t0}ms`);
          resolve({ statusCode: 200, headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(error ? { races: merged, error } : { races: merged }) });
        }).catch(() => {
          console.log(`[timing-race] falha ao mesclar cache, retornando so provas da busca ao vivo: ${races.length}`);
          resolve({ statusCode: 200, headers: { 'Content-Type': 'application/json' },
    ...
    req.on('error', (e) => {
      console.log(`[timing-race] excecao de rede apos ${Date.now() - _t0}ms:`, e.message);
      resolve({ statusCode: 200, headers: { 'Content-Type': 'application/json' },
